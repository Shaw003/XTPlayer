...
//  XTCountdown.swift
//
//  Created by Shaw on 2019/2/13.
//  Copyright © 2019年 Shaw. All rights reserved.
//

import UIKit

public let xt_countdown = XTCountdown.instance
...
public struct XTCountdownFunction: OptionSet {
    ...
    /** 定时器暂停，只在本类内可使用*/
    fileprivate static let pause = XTCountdownFunction(rawValue: 1 << 1)
    /** 定时器运行，只在本类内可使用*/
    fileprivate static let timing = XTCountdownFunction(rawValue: 1 << 2)
    /** 持久化，是否需要杀死程序后仍然开启计时*/
    ...
    @discardableResult
    public mutating func add(_ function: XTCountdownFunction) -> XTCountdownFunction {
        if function.contains(.pause) {
            self.xor(.timing)
        } else if function.contains(.timing) {
            self.xor(.pause)
        }
        self = XTCountdownFunction.init(rawValue: self.rawValue | function.rawValue)
        return self
    }
    
    @discardableResult
    /** 异或操作*/
    public mutating func xor(_ function: XTCountdownFunction) -> XTCountdownFunction {
...
extension Array where Element == XTCountdownFunction {
    
    @discardableResult
    mutating func add(_ function: XTCountdownFunction) -> [XTCountdownFunction] {
    ...
    @discardableResult
    /** 异或操作*/
    mutating func xor(_ function: XTCountdownFunction) -> [XTCountdownFunction] {
...
public class XTCountdown: NSObject {
    
    static let instance = XTCountdown()
    ...
    public func checkCountdown(key: String, progressHandler: ((inout CountdownDetailInfo) -> ())? = nil) -> (Bool, Bool) {
        ...
        let stateFunction = XTCountdownFunction.init(rawValue: state)
...
//            debugPrint("read：计时器在暂停中")
    ...
    public func pauseOrResumeCountdown(key: String, forcePause: Bool? = nil) -> CountdownDetailInfo? {
        ...
        let function = XTCountdownFunction.init(rawValue: state)
...
//            debugPrint("write：计时器状态为计时中")
    ...
    /// - Returns: 计时器元组
    @discardableResult
    public func startCountdown(key: String, seconds: UInt, function: XTCountdownFunction = .default, onlyStart: Bool = false, progressHandler progress: ((inout CountdownDetailInfo) -> ())? = nil) -> CountdownDetailInfo? {
